...
$version  = Get-Content (Join-Path $root "VERSION") | Select-Object -First 1
$version  = $version.Trim().Split(" ")[0]
$stem     = "LSP_Simple_Open_DRT_$version"
$plugin   = Join-Path $root "release\$stem.ofx.bundle\Contents\Win64\$stem.ofx"
$installer= Join-Path $PSScriptRoot "installer\LSP_Simple_Open_DRT_v$version`_Windows_cuda_opencl_Installer.exe"
...
if (!(Test-Path $pfx)) {
  ...
  $cert = New-SelfSignedCertificate -Type CodeSigningCert -Subject "CN=Loïs Plagnard" -CertStoreLocation "Cert:\CurrentUser\My" -HashAlgorithm SHA256

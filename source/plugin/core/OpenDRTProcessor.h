...
#if defined(OFX_SUPPORTS_CUDARENDER) && !defined(SIMPLE_OPENDRT_VIEWER_CPU_ONLY)
#define SIMPLE_OPENDRT_HAS_CUDA 1
#endif

#if (defined(_WIN32) || defined(__linux__)) && !defined(SIMPLE_OPENDRT_VIEWER_CPU_ONLY)
#define SIMPLE_OPENDRT_HAS_OPENCL 1
#endif
...
#if defined(SIMPLE_OPENDRT_HAS_OPENCL)
...
#if defined(__APPLE__) && !defined(SIMPLE_OPENDRT_VIEWER_CPU_ONLY)
...
class OpenDRTProcessor {
 public:
...
#elif defined(SIMPLE_OPENDRT_HAS_OPENCL)
    if (!openclDisableEnabled_ && openclAvailableCached()) {
      if (renderOpenCL(src, dst, width, height, srcRowBytes, dstRowBytes)) return true;
      debugLog("OpenCL path failed, falling back to CPU.");
    }
  ...
  bool renderCUDAHostBuffers(
    ...
    if (evStart != nullptr && evAfterKernel != nullptr && (!usingHostStream || hostCudaForceSyncEnabled_)) {
      ...
      debugLog(msg.c_str());
      if (perfLogEnabled_) {
        std::fprintf(stderr, "[Simple_Open_DRT][PERF] %s\n", msg.c_str());
#if defined(_WIN32)
        ...
        if (base != nullptr && base[0] != '\0') {
          ...
          if (ofs.is_open()) ofs << "[Simple_Open_DRT][PERF] " << msg << "\n";
        }
#endif
      }
    }
 ...
 private:
  ...
  void debugLog(const char* msg) const {
    ...
    appendMacLogLine(std::string("[Simple_Open_DRT] ") + msg);
  }
  ...
  void perfLogStage(const char* label, const std::chrono::steady_clock::time_point& start) const {
    ...
    {
      char buf[256];
      std::snprintf(buf, sizeof(buf), "[Simple_Open_DRT][PERF] %s: %.3f ms", label, ms);
    ...
    if (!logPath.empty()) {
      ...
      if (ofs.is_open()) {
        ofs << "[Simple_Open_DRT][PERF] " << label << ": " << ms << " ms\n";
      }
    }
#endif
  }
  ...
  void initRuntimeFlags() {
    debugLogEnabled_ = envFlagEnabled("SIMPLE_OPENDRT_DEBUG_LOG");
...
#endif
#if defined(SIMPLE_OPENDRT_HAS_OPENCL)
    openclForceEnabled_ = envFlagEnabled("SIMPLE_OPENDRT_FORCE_OPENCL");
    openclDisableEnabled_ = envFlagEnabled("SIMPLE_OPENDRT_DISABLE_OPENCL");
    openclDisable2DCopyEnabled_ = envFlagEnabled("SIMPLE_OPENDRT_OPENCL_DISABLE_2D_COPY");
    // Note-to-self:
    ...
    const char* openclFallbackEnv = std::getenv("SIMPLE_OPENDRT_OPENCL_EXTERNAL_KERNEL_FALLBACK");
...
#endif
    disableDerivedEnabled_ = envFlagEnabled("SIMPLE_OPENDRT_DISABLE_DERIVED");
  }
  ...
  bool initializeOpenCLRuntime() {
    ...
    auto printBuildLog = [&]() {
      ...
      if (logSize > 1) {
        ...
        if (debugLogEnabled_) std::fprintf(stderr, "[Simple_Open_DRT] OpenCL build log:\n%s\n", log.data());
      }
    };
    ...
    auto buildProgramFromSource = [&](const char* src, size_t len, const char* sourceMode) -> bool {
      ...
      if (err == CL_SUCCESS) {
        if (debugLogEnabled_) std::fprintf(stderr, "[Simple_Open_DRT] OpenCL kernel source mode: %s\n", sourceMode);
        return true;
      }
    ...
    if (!programReady && openclExternalKernelFallbackEnabled_) {
      ...
      if (ifs.is_open()) {
        ...
        std::fprintf(stderr, "[Simple_Open_DRT] OpenCL external fallback source not found: %s\n", path.c_str());
      }
    }

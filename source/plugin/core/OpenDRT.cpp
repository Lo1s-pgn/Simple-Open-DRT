...
#define kPluginDescription "Simple Open DRT OFX by Loïs Plagnard, based on OpenDRT by Jed Smith"
...
// - SIMPLE_OPENDRT_RENDER_MODE=HOST|AUTO -> host preferred
// - SIMPLE_OPENDRT_RENDER_MODE=INTERNAL  -> internal only
...
// - SIMPLE_OPENDRT_METAL_RENDER_MODE=HOST|AUTO -> host preferred
// - SIMPLE_OPENDRT_METAL_RENDER_MODE=INTERNAL  -> internal-only path
...
void cubeViewerDebugLog(const std::string& msg) {
  ...
  const std::string line = std::string("[Simple_Open_DRT] ") + msg;
...
void perfLog(const char* stage, const std::chrono::steady_clock::time_point& start) {
  ...
  if (!pathInit) {
    ...
    if (home && *home) {
      ...
      logPath = "/tmp/Simple_Open_DRT_perf.log";
    }
  }
  if (!logPath.empty()) {
    FILE* f = std::fopen(logPath.c_str(), "a");
...
std::string cubeViewerPipeName() {
  const char* env = std::getenv("SIMPLE_OPENDRT_CUBE_VIEWER_PIPE");
  if (env && env[0] != '\0') return std::string(env);
#if defined(_WIN32)
  return "\\\\.\\pipe\\LSP_Simple_Open_DRT_CubeViewer";
#else
  return "/tmp/lsp_simple_open_drt_cube_viewer.sock";
#endif
}

std::string cubeViewerExeName() {
#if defined(_WIN32)
  return "LSP_Simple_Open_DRT_CubeViewer.exe";
#else
  return "LSP_Simple_Open_DRT_CubeViewer";
#endif
}
...
std::vector<std::string> cubeViewerExeCandidates() {
  ...
  const char* env = std::getenv("SIMPLE_OPENDRT_CUBE_VIEWER_EXE");
  if (env && env[0] != '\0') {
...
#if defined(__APPLE__)
      out.push_back(macAppExecutablePath(joinPath(joinPath(contentsDir, "Resources"), "LSP_Simple_Open_DRT_CubeViewer.app"), exeName));
#endif
    }
  }
...
bool confirmDeleteDialog(const std::string& presetName) {
  ...
  return MessageBoxA(nullptr, msg.c_str(), "Simple_Open_DRT", MB_ICONWARNING | MB_YESNO) == IDYES;
}
...
std::string pickOpenJsonFilePath() {
  return execAndRead("osascript -e 'POSIX path of (choose file with prompt \"Import Simple_Open_DRT preset\" of type {\"public.json\"})' 2>/dev/null");
}
...
std::string pickOpenJsonFilePath() {
  if (linuxCommandExists("zenity")) {
    return execAndReadLinux("zenity --file-selection --title=\"Import Simple_Open_DRT preset\" --file-filter=\"*.json\" 2>/dev/null");
  }
...
bool confirmDeleteDialog(const std::string& presetName) {
  if (linuxCommandExists("zenity")) {
    ...
    const std::string cmd =
        "zenity --question --title=\"Simple_Open_DRT\" --text=\"Delete preset '" + safe + "'? This cannot be undone.\" 2>/dev/null";
    return std::system(cmd.c_str()) == 0;
  }
  if (linuxCommandExists("kdialog")) {
    std::string safe = presetName;
  ...
  std::fprintf(stderr, "[Simple_Open_DRT] Linux fallback: delete confirmation unavailable for preset '%s'.\n", presetName.c_str());
  return false;
}
...
DeleteTarget choosePresetTargetDialog(const char* actionVerb) {
  ...
  if (linuxCommandExists("zenity")) {
    ...
    for (char& c : safe) if (c == '"') c = '\'';
    const std::string cmd =
        "zenity --question --title=\"Simple_Open_DRT\" "
  ...
  std::fprintf(
      stderr,
      "[Simple_Open_DRT] Linux fallback: preset target chooser unavailable for action '%s'.\n",
      action.c_str());
  return DeleteTarget::Cancel;
}
...
void migrateLegacyV1IfNeededLocked() {
  ...
  if (header != "SIMPLE_OPENDRT_USER_PRESETS_V1") return;
...
int customLookPresetIndex() {
  std::lock_guard<std::mutex> lock(userPresetMutex());
  ensureUserPresetStoreLoadedLocked();
  return kBuiltInLookPresetCount + static_cast<int>(userPresetStore().lookPresets.size());
}

int customTonescalePresetIndex() {
  std::lock_guard<std::mutex> lock(userPresetMutex());
  ensureUserPresetStoreLoadedLocked();
  return kBuiltInTonescalePresetCount + static_cast<int>(userPresetStore().tonescalePresets.size());
}
...
void render(const OFX::RenderArguments& args) override {
              ...
              if (eSrc == cudaSuccess && eDst == cudaSuccess) {
                ...
                if (eSync == cudaSuccess) {
                  ...
                  if (buildCubeViewerInputCloudPayload(
                    ...
                    std::fprintf(stderr, "[Simple_Open_DRT] Cube input cloud build failed after CUDA host readback.\n");
                  }
                } else if (debugLogEnabled()) {
                  std::fprintf(stderr, "[Simple_Open_DRT] Cube input cloud CUDA sync failed: %s\n", cudaGetErrorString(eSync));
                }
              } else if (debugLogEnabled()) {
                std::fprintf(
                    stderr,
                    "[Simple_Open_DRT] Cube input cloud CUDA copy failed: src=%s dst=%s\n",
        ...
        if (needHostMetalReadback) {
          ...
          if (hostMetalRendered) {
            ...
            if (buildCubeViewerInputCloudPayload(
              ...
              std::fprintf(stderr, "[Simple_Open_DRT] Cube input cloud build failed after explicit host-Metal readback.\n");
            }
    ...
    if (bypassHostMetalForCubeViewerCloud && debugLogEnabled()) {
      std::fprintf(stderr, "[Simple_Open_DRT] Bypassing host Metal for cube viewer cloud frame (steady-state).\n");
    }
#endif
    ...
    if (rendered && (needHostReadableInputCloud || canUpdateInputCloudCache)) {
      ...
      if (buildCubeViewerInputCloudPayload(
        ...
        std::fprintf(stderr, "[Simple_Open_DRT] Cube input cloud build failed on host-readable render path.\n");
      }
    }
...
void changedParam(const OFX::InstanceChangedArgs& args, const std::string& paramName) override {
    try {
      ...
      // Tonescale and creative white selectors are automatically aligned to the look.
      ...
      // Tonescale preset is explicit and directly applied.
      ...
      if (paramName == "reset_tonescale" ||
        ...
        if (paramName == "reset_look_settings") {
          ...
          setChoice("creativeWhitePreset", expected.cwp);
      ...
      if (paramName == "userPresetCombined") {
        ...
        const int rel = idx - 1;
        if (rel < 0 || rel >= static_cast<int>(names.size())) return;
        const std::filesystem::path path = combinedUserPresetDirPath() / (names[static_cast<size_t>(rel)] + ".xml");
        std::string presetName;
        LookPresetValues look{};
        TonescalePresetValues tone{};
        if (!readCombinedPresetXmlFile(path, &presetName, &look, &tone)) return;
        FlagScope scope(suppressParamChanged_);
      ...
      if (paramName == "userPresetExportTonescale" || paramName == "userPresetExportXml") {
        ...
        const std::string name = sanitizePresetName(path.stem().string(), "Preset");
        if (std::filesystem::exists(path) && !confirmOverwriteDialog(name)) return;
        const LookPresetValues look = captureCurrentLookValues(args.time);
        const TonescalePresetValues tone = captureCurrentTonescaleValues(args.time);
        if (!writeCombinedPresetXmlFile(path, name, look, tone)) return;
      ...
      if (paramName == "userPresetImport" || paramName == "userPresetImportXml") {
        ...
        if (std::filesystem::exists(dst) && !confirmOverwriteDialog(name)) return;
        if (!writeCombinedPresetXmlFile(dst, name, look, tone)) return;
        FlagScope scope(suppressParamChanged_);
        writeLookValuesToParams(look, *this);
        writeTonescaleValuesToParams(tone, *this);
        syncCombinedPresetMenuFromDisk(args.time, 0);
      ...
      if (paramName == "userPresetOpenFolder" || paramName == "userPresetOpenFolderXml") {
        (void)openExternalUrl(std::string("file://") + combinedUserPresetDirPath().string());
        return;
      }

      if (paramName == "userPresetRefresh" || paramName == "userPresetRefreshXml") {
      ...
      if (paramName == "userPresetOpenFolder") {
        (void)openExternalUrl(std::string("file://") + userPresetDirPath().string());
        return;
      }
 ...
 private:
  ...
  void applyPresetMenuModifiedLabels(double t, bool lookModified, bool tonescaleModified, bool creativeWhiteModified, bool displayEncodingModified) {
    ...
    const int cwpIdx = getChoice("creativeWhitePreset", t, 2);
    ...
    if (lookParam) {
      ...
      if (!base.empty() && lookModified) lookParam->setOption(lookIdx, base + " (custom)");
    }
    if (toneParam) {
      const std::string base = tonescaleBaseMenuName(toneIdx);
      if (!base.empty() && tonescaleModified) toneParam->setOption(toneIdx, base + " (custom)");
    }
    if (cwpParam) {
      const std::string base = creativeWhiteBaseMenuName(cwpIdx);
      if (!base.empty() && creativeWhiteModified) cwpParam->setOption(cwpIdx, base + " (custom)");
    }
    if (displayParam) {
      const std::string base = displayEncodingBaseMenuName(displayIdx);
      if (!base.empty() && displayEncodingModified) displayParam->setOption(displayIdx, base + " (custom)");
    }
  ...
  void rebuildLookPresetMenuOptions(int preferredIndex) {
    ...
    const int maxIndex = kBuiltInLookPresetCount + userCount;
  ...
  void rebuildTonescalePresetMenuOptions(int preferredIndex) {
    ...
    p->appendOption("(custom)");
    const int maxIndex = kBuiltInTonescalePresetCount + userCount;
    const int clamped = preferredIndex < 0 ? 0 : (preferredIndex > maxIndex ? maxIndex : preferredIndex);
    p->setValue(clamped);
  }
  ...
  void syncPresetMenusFromDisk(double t, int preferredLookIndex, int preferredToneIndex) {
    ...
    int tonePreferred = preferredToneIndex;
    {
      ...
      const int maxLook = kBuiltInLookPresetCount + static_cast<int>(userPresetStore().lookPresets.size());
      const int maxTone = kBuiltInTonescalePresetCount + static_cast<int>(userPresetStore().tonescalePresets.size());
...
void pushCubeViewerUpdate(double time, const std::string& changedParam, bool forceSnapshot = false) {
  ...
  bool connectCubeViewerWithRetry(int attempts, int sleepMs) {
    const std::string hello = "{\"type\":\"hello\",\"protocolVersion\":1,\"plugin\":\"Simple_Open_DRT\"}";
...
void openCubeViewerSession(double time) {
    ...
    if (!launchCubeViewerProcessAsync(&launchError, &launchedPath, &launchedPid)) {
      ...
      if (debugLogEnabled()) {
        std::fprintf(stderr, "[Simple_Open_DRT] Cube viewer launch failed: %s\n", launchError.c_str());
      }
      return;
    }
    cubeViewerProcessId_ = launchedPid;
    if (debugLogEnabled()) {
      std::fprintf(stderr, "[Simple_Open_DRT] Cube viewer launched from: %s\n", launchedPath.c_str());
    }
...
static const std::string nameWithVersion = "LSP - Simple Open DRT v1.1.1";
...
void describeInContext(OFX::ImageEffectDescriptor& d, OFX::ContextEnum) override {
    ...
    if (const char* hint = tooltipForParam("tn_enable")) tnEnable->setHint(hint);
    ...
    if (const char* hint = tooltipForParam("rs_enable")) rsEnable->setHint(hint);
    ...
    if (const char* hint = tooltipForParam("ptm_enable")) ptmEnable->setHint(hint);
    ...
    if (const char* hint = tooltipForParam("pt_enable")) ptEnable->setHint(hint);
    ...
    if (const char* hint = tooltipForParam("brl_enable")) brlEnable->setHint(hint);
    ...
    if (const char* hint = tooltipForParam("hc_enable")) hcEnable->setHint(hint);
    ...
    userPresetName->setLabel("Preset Name");
    userPresetName->setDefault("");
    userPresetName->setParent(*grpUserPresetsRoot);
    userPresetName->setIsSecret(true);
    userPresetName->setEnabled(false);
    ...
    userLookSave->setIsSecret(true);
    userLookSave->setEnabled(false);
    ...
    userTonescaleSave->setIsSecret(true);
    userTonescaleSave->setEnabled(false);
    ...
    userPresetImport->setIsSecret(true);
    userPresetImport->setEnabled(false);

    auto* userPresetExportLook = d.definePushButtonParam("userPresetExportLook");
    userPresetExportLook->setLabel("Export Look Preset...");
    userPresetExportLook->setParent(*grpUserPresetsRoot);
    userPresetExportLook->setIsSecret(true);
    userPresetExportLook->setEnabled(false);

    auto* userPresetExportTonescale = d.definePushButtonParam("userPresetExportTonescale");
    userPresetExportTonescale->setLabel("Export Preset");
    userPresetExportTonescale->setParent(*grpUserPresetsRoot);
    userPresetExportTonescale->setIsSecret(true);
    userPresetExportTonescale->setEnabled(false);
    ...
    userPresetUpdateCurrent->setIsSecret(true);
    ...
    userPresetDeleteCurrent->setIsSecret(true);
    ...
    userPresetRenameCurrent->setIsSecret(true);
    ...
    userPresetRefresh->setIsSecret(true);
    userPresetRefresh->setEnabled(false);

    auto* userPresetOpenFolder = d.definePushButtonParam("userPresetOpenFolder");
    userPresetOpenFolder->setLabel("Open Folder");
    userPresetOpenFolder->setParent(*grpUserPresetsRoot);
    userPresetOpenFolder->setIsSecret(true);
    userPresetOpenFolder->setEnabled(false);
    ...
    auto* userPresetOpenFolderXml = d.definePushButtonParam("userPresetOpenFolderXml");
    userPresetOpenFolderXml->setLabel("Open Folder");
    userPresetOpenFolderXml->setParent(*grpUserPresetsRoot);
    ...
    closeCubeViewer->setIsSecret(true);
    closeCubeViewer->setEnabled(false);
    ...
    cubeViewerLive->setIsSecret(true);
    cubeViewerLive->setEnabled(false);
    ...
    cubeViewerSource->setParent(*grpCubeViewer);
    ...
    cubeViewerOnTop->setIsSecret(true);
    cubeViewerOnTop->setEnabled(false);
    ...
    cubeViewerQuality->setIsSecret(true);
    cubeViewerQuality->setEnabled(false);
    ...
    cubeViewerShowOverflow->setIsSecret(true);
    cubeViewerShowOverflow->setEnabled(false);
    ...
    cubeViewerHighlightOverflow->setIsSecret(true);
    cubeViewerHighlightOverflow->setEnabled(false);
    ...
    cubeViewerStatus->setIsSecret(true);
    if (const char* hint = tooltipForParam("cubeViewerStatus")) cubeViewerStatus->setHint(hint);

    // Keep quick controls under hidden cube-viewer group to avoid host UI separator artifacts.
    auto* openCubeViewer = d.definePushButtonParam("openCubeViewer");
    openCubeViewer->setLabel("Open 3D Cube Viewer");
    openCubeViewer->setParent(*grpCubeViewer);
    openCubeViewer->setIsSecret(true);
    openCubeViewer->setEnabled(false);
    ...
    cubeViewerIdentity->setParent(*grpCubeViewer);
    cubeViewerIdentity->setIsSecret(true);
    cubeViewerIdentity->setEnabled(false);
    ...
    cubeViewerPlotLinear->setParent(*grpCubeViewer);
    cubeViewerPlotLinear->setIsSecret(true);
    cubeViewerPlotLinear->setEnabled(false);

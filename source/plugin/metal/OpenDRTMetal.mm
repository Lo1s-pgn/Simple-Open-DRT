...
bool perfLogEnabled() {
  static const bool enabled = envFlagEnabled("SIMPLE_OPENDRT_PERF_LOG");
  return enabled;
}

bool debugLogEnabled() {
  static const bool enabled = envFlagEnabled("SIMPLE_OPENDRT_DEBUG_LOG");
  return enabled;
}

bool shouldSerializeRender() {
  static const bool enabled = envFlagEnabled("SIMPLE_OPENDRT_METAL_SERIALIZE");
  return enabled;
}

bool forceHostMetalWait() {
  static const bool enabled = envFlagEnabled("SIMPLE_OPENDRT_HOST_METAL_FORCE_WAIT");
  return enabled;
}

bool disableMetal2DCopy() {
  static const bool enabled = envFlagEnabled("SIMPLE_OPENDRT_DISABLE_METAL_2D_COPY");
  return enabled;
}

bool metalDiagEnabled() {
  static const bool enabled = envFlagEnabled("SIMPLE_OPENDRT_METAL_DIAG");
  return enabled;
}
...
HostOffsetMode hostOffsetMode() {
  static const HostOffsetMode mode = []() {
    const char* v = std::getenv("SIMPLE_OPENDRT_HOST_METAL_OFFSET_MODE");
    if (v != nullptr && v[0] != '\0') {
      std::string s(v);
...
bool useHostIntermediateDst() {
  static const bool enabled = []() {
    const char* v = std::getenv("SIMPLE_OPENDRT_HOST_METAL_INTERMEDIATE_DST");
    if (v != nullptr && v[0] != '\0') {
      return !(v[0] == '0' && v[1] == '\0');
    }
#if defined(__APPLE__) && (defined(__aarch64__) || defined(__arm64__))
...
void debugLog(const char* msg) {
  ...
  std::fprintf(stderr, "[Simple_Open_DRT][Metal] %s\n", msg);
  ...
  if (!logPath.empty()) {
    ...
    if (f != nullptr) {
      std::fprintf(f, "[Simple_Open_DRT][Metal] %s\n", msg);
      std::fclose(f);
    }
  }
}
...
void diagLog(const std::string& msg) {
  ...
  std::fprintf(stderr, "[Simple_Open_DRT][MetalDiag] %s\n", msg.c_str());
  static std::mutex logMutex;
  static bool pathInit = false;
  static std::string logPath;
  if (!pathInit) {
  ...
  if (!logPath.empty()) {
    ...
    FILE* f = std::fopen(logPath.c_str(), "a");
    if (f != nullptr) {
      std::fprintf(f, "[Simple_Open_DRT][MetalDiag] %s\n", msg.c_str());
      std::fclose(f);
    }
  }
}
...
void perfLogStage(const char* stage, const std::chrono::steady_clock::time_point& start) {
  ...
  std::fprintf(stderr, "[Simple_Open_DRT][PERF][Metal] %s: %.3f ms\n", stage, ms);
  static std::mutex logMutex;
  static bool pathInit = false;
  static std::string logPath;
  if (!pathInit) {
  ...
  if (!logPath.empty()) {
    ...
    FILE* f = std::fopen(logPath.c_str(), "a");
    if (f != nullptr) {
      std::fprintf(f, "[Simple_Open_DRT][PERF][Metal] %s: %.3f ms\n", stage, ms);
      std::fclose(f);
    }
  }
}
...
bool finishInFlightHostCommandIfNeeded(MetalContext& ctx, ThreadBuffers& buffers, const char* reason) {
  ...
  if (!ok) {
    ...
    if (inFlight.error != nil) {
      NSLog(@"Simple_Open_DRT Metal host in-flight failure (%s): %@", reason, inFlight.error.localizedDescription);
    } else {
      NSLog(@"Simple_Open_DRT Metal host in-flight failure (%s): unknown error", reason);
    }
    if (metalDiagEnabled()) {
...
bool initializePipelineForDevice(id<MTLDevice> device, id<MTLCommandQueue> defaultQueue) {
  ...
  if (debugLogEnabled()) {
    ...
    NSLog(@"Simple_Open_DRT Metal: runtime macOS %ld.%ld.%ld, metallib path: %s",
  ...
  if (library == nil) {
    if (error != nil) {
      NSLog(@"Simple_Open_DRT Metal: failed to load metallib: %@", error.localizedDescription);
    }
    return false;
  }
  ...
  if (function == nil) {
    ...
    if (names != nil && names.count > 0) {
      ...
      NSLog(@"Simple_Open_DRT Metal: available metallib functions: %@", joined);
    } else {
      NSLog(@"Simple_Open_DRT Metal: metallib contains no discoverable functions");
    }
    NSLog(@"Simple_Open_DRT Metal: OpenDRTKernel entry not found in metallib");
    return false;
  }
  ...
  if (ctx.pipeline == nil) {
    if (error != nil) {
      NSLog(@"Simple_Open_DRT Metal: failed to create compute pipeline: %@", error.localizedDescription);
    }
    return false;
  }
...
static bool renderImpl(
  ...
  auto chooseThreadsPerThreadgroup = [&]() -> MTLSize {
    const char* env = std::getenv("SIMPLE_OPENDRT_METAL_BLOCK");
  ...
  if (cmd.status != MTLCommandBufferStatusCompleted) {
    if (cmd.error != nil) {
      NSLog(@"Simple_Open_DRT Metal: command buffer failed: %@", cmd.error.localizedDescription);
    }
...
bool renderHost(
  ...
  switch (offsetMode) {
    ...
    case HostOffsetMode::FallbackAmbiguous:
      if (ambiguousOffset) {
        if (debugLogEnabled()) {
          std::fprintf(
              stderr,
              "[Simple_Open_DRT][Metal] Host offset ambiguous (origin=%d,%d rowBytes=%zu/%zu len=%zu/%zu). Falling back.\n",
              originX,
              originY,
              srcRowBytes,
              dstRowBytes,
    ...
    default:
      ...
      if (ambiguousOffset && debugLogEnabled()) {
        std::fprintf(
            stderr,
            "[Simple_Open_DRT][Metal] Host offset ambiguous -> AUTO chose ORIGIN (origin=%d,%d).\n",
            originX,
            originY);
      }
      break;
  ...
  if (forceHostMetalWait()) {
    ...
    if (cmd.status != MTLCommandBufferStatusCompleted) {
      if (cmd.error != nil) {
        NSLog(@"Simple_Open_DRT Metal host: command buffer failed: %@", cmd.error.localizedDescription);
      }
...
bool renderHostReadback(
  ...
  if (cmd.status != MTLCommandBufferStatusCompleted) {
    if (cmd.error != nil) {
      NSLog(@"Simple_Open_DRT Metal host readback failed: %@", cmd.error.localizedDescription);
    }

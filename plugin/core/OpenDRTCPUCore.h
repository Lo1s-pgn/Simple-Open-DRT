...
inline float3 linearize(float3 rgb, int tf) {
  // Match OpenDRT OFX transfer-function enum order (same as GPU backends).
  if (tf == 0) return rgb; // Linear
...
inline float3 transformPixel(int width, int height, int x, int y, float3 rgb, const OpenDRTParams* p, const OpenDRTDerivedParams* d) {
  ...
  // Match OpenDRT OFX input-gamut enum order (same as GPU backends).
  if (p->in_gamut == 0) in_to_xyz = identity();
  else if (p->in_gamut == 1) in_to_xyz = matrix_ap0_to_xyz;

...
static constexpr std::array<const char*, 13> kTonescalePresetNames = {
...
static constexpr const char* kOpenDRTPortVersion = PLUGIN_VERSION_STR;
...
// Tonescale preset table order matches kTonescalePresetNames.
...
inline void writeCreativeWhitePresetToParams(int creativeWhitePresetIndex, OFX::ImageEffect& fx) {
  ...
  setIntIfPresent(fx, "cwp", creativeWhitePresetIndex);
}
...
inline OpenDRTParams resolveParams(const OpenDRTRawValues& r) {
  ...
  if (r.creativeWhitePreset >= 0) {
    // OFX menu is explicit D93..D50 (0..5) with no "use look preset" entry.
    p.cwp = r.creativeWhitePreset;
  }
...
inline void writePresetToParams(int presetIndex, OFX::ImageEffect& fx) {
  ...
  setStringIfPresent(fx, "baseTonescaleLabel", baseTonescaleLabelForLook(idx));
  setStringIfPresent(fx, "baseWhitepointLabel", baseWhitepointLabelForLook(idx));
}
